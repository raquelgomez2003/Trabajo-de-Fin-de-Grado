...
Calls back into the main app when the user confirms.
"""
...
class SetupWindow(ctk.CTkToplevel):
    """
    Modal-style setup wizard.
    On confirm, calls on_confirm(config_dict) where config_dict contains:
      {
        "folder":    str,               # base dataset folder
        "device":    "Biopac"|"Empatica",
        "n_subjects": int,
        "signals":   [str, ...],        # selected signal types
        "fs":        {sig: float, ...}, # sampling freq per signal
      }
    """
    ...
    def _refresh_signals(self):
        """Rebuild the signal checkboxes and Fs entries for the selected device."""
        ...
        ctk.CTkLabel(hdr, text="Signal", width=120, font=("Arial", 11, "bold")).pack(side="left")
        ctk.CTkLabel(hdr, text="Include", width=70,  font=("Arial", 11, "bold")).pack(side="left")
        ctk.CTkLabel(hdr, text="Fs (Hz)",  width=100, font=("Arial", 11, "bold")).pack(side="left")
        ...
        for sig in DEVICE_SIGNALS.get(device, []):
            ...
            self._sig_vars[sig] = bvar

            fsvar = ctk.StringVar(value=str(default_fs.get(sig, 2000)))
            self._fs_vars[sig] = fsvar

            ctk.CTkLabel(row, text=sig, width=120).pack(side="left")
            ctk.CTkCheckBox(row, variable=bvar, text="", width=70).pack(side="left")
            ctk.CTkEntry(row, textvariable=fsvar, width=90).pack(side="left", padx=6)
    ...
    def _confirm(self):
        ...
        if not selected:
            messagebox.showerror("No signals",
                                 "Select at least one signal to load.")
            return
        ...
        for sig in selected:
            try:
                ...
                messagebox.showerror("Invalid Fs",
                                     f"Sampling frequency for {sig} is not valid.")
                return
